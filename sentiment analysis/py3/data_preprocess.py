...
def get_dataset(path, stop_words):
	...
	train_set_pos_x = list(filter(lambda x: len(x)>10 and len(x)<1000, train_set_pos_x))
	...
	train_set_neg_x = list(filter(lambda x: len(x)>10 and len(x)<1000, train_set_neg_x))
	...
	test_set_pos_x = list(filter(lambda x: len(x)>10 and len(x)<1000, test_set_pos_x))
	...
	test_set_neg_x = list(filter(lambda x: len(x)>10 and len(x)<1000, test_set_neg_x))
...
def build_word2vec(bin, vocab):
	...
	with open(bin, "rb") as f:
		...
		for line in range(vocab_size):
			word = []
			while True:
				ch = f.read(1).decode('unicode_escape')
				if ch == ' ':
					word = ''.join(word)
...
def build_dictionary(vocab, word2vec):
	...
	word2idx = {}
	for idx, (word, rep) in enumerate(word2vec.items()):
		w[idx+1] = rep 
		word2idx[word] = idx+1
	return w, word2idx
	
if __name__ == "__main__":
	stop_words = list(map(lambda x : x.strip().lower(), list(stopwords.words("english"))))
	print(type(stop_words))
	with open("stop_words.txt", "rb") as fin:
		stop_words = stop_words + [line.strip().lower() for line in fin]
	#print(stop_words)

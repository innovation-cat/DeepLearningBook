...
def momentum(cost, params, lr, mu=0.5):
	...
	gparams = T.grad(cost, params)
	updates = []
	for p, g, v in itertools.zip(params, gparams, velocity):
		dir = mu * v + lr * g  
...
def nesterov_momentum(cost, params, lr, mu=0.5):
	...
	for param, gparam, velocity in itertools.zip(params, gparams, velocitys):
		#p = param - mu*velocity
...
def adagrad(cost, params, lr):
	...
	for param, gparam, acc in itertools.zip(params, gparams, accs):
...
def adadelta(cost, params, lr, rho=0.95):
	...
	for param, gparam, acc, delta_acc in itertools.zip(params, gparams, accs, delta_accs):

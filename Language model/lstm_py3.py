...
def adadelta(lr, tparams, grads, x, mask, y, cost):
	zipped_grads = [theano.shared(p.get_value()*0.0, name='%s_grad'%k) for k, p in tparams.items()]
	
	running_up2 = [theano.shared(p.get_value()*0.0, name='%s_rup2'%k) for k, p in tparams.items()]
	
	running_grads2 = [theano.shared(p.get_value()*0.0, name='%s_rgrad2'%k) for k, p in tparams.items()]
	...
	params_updates = [(p, p+d) for p, d in list(zip(list(tparams.values()), dir))]
...
class LSTM:
	def __init__ (self, options):
		...
		grads = T.grad(cost, list(self.tparams.values()))
		...
		#)
		updates = [(p, p-lr*g) for p, g in list(zip(list(self.tparams.values()), grads))]
...
if __name__ == "__main__":
	...
	with open("small_dataset.csv", "r", encoding='utf-8') as fin:
		reader = csv.reader(fin)
		next(reader)
		# sentence tokenize
		sentences = itertools.chain(*[(nltk.sent_tokenize(line[0].strip().lower())) for line in reader])
	...
	print(len(words))
	words = list(filter(lambda x:len(x)>5 and len(x)<1000, words))
	print(len(words))
	...
	if len(words)%batch_size==0:
		...
		n_batch = int(len(words)/batch_size)+1
	print("len_word: %d, batch_size: %d, n_batch: %d" % (len(words), batch_size, n_batch))
	with open("cost_small.txt", "w") as fout:
		for ep in range(100):
